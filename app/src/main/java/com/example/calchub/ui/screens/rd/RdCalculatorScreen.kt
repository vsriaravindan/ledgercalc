package com.example.calchub.ui.screens.rd
...
/**
 * Screen for Recurring Deposit (RD) calculation.
 * Calculates maturity spread based on monthly deposits and interest rate.
 *
 * @param onBackClick Callback triggered when the back button is clicked.
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun RdCalculatorScreen(onBackClick: () -> Unit) {
    var monthlyDeposit by remember { mutableDoubleStateOf(5000.0) }
    var interestRate by remember { mutableDoubleStateOf(6.5) }
    var timePeriodYears by remember { mutableDoubleStateOf(5.0) }

    val results = CalculatorLogic.calculateRD(monthlyDeposit, interestRate, timePeriodYears)
